...
async function initGlobalObjects() {
  "file:" != window.location.protocol && (await loadIncludes()),
    ...
    (fileTreeItems = Array.from(
      document.querySelectorAll(".tree-container.file-tree .tree-item")
    )),
    (sidebars = []),
    (sidebarGutters = []),
    (sidebarCollapseIcons = []),
...
async function initializePage() {
  ...
  (canvasNodes = e.length > 0 ? e : canvasNodes),
    ...
    (viewContent =
      ...
      viewContent),
    (outlineTreeItems = Array.from(
      document.querySelectorAll(".tree-container.outline-tree .tree-item")
    )),
    fullyInitialized ||
    ...
    ("video" != embedType &&
      ...
      ? rightSidebar &&
        ...
        (rightSidebar.collapse(!1), (rightSidebar.temporarilyCollapsed = !1))
...
function initializeDocumentTypes(e) {
  e.querySelector(".document-container > .markdown-preview-view")
    ? (documentType = "markdown")
    : e.querySelector(".canvas-wrapper")
    ? (documentType = "canvas")
    : ((documentType = "custom"),
      e.querySelector(".kanban-plugin")
        ? (customType = "kanban")
        : e.querySelector(".excalidraw-plugin") && (customType = "excalidraw"));
}
...
(window.onload = async function () {
  await initializePage(),
    ...
    setActiveDocument(loadedURL, !0, !1, !1),
    (fullyInitialized = !0);
}),
...
function onResize(e = !1) {
  ...
  function o(e) {
    ...
    return (t > e && null == lastScreenWidth) || (t > e && lastScreenWidth < e);
  }
  function n(e) {
    let t = window.innerWidth;
    return (t < e && null == lastScreenWidth) || (t < e && lastScreenWidth > e);
  ...
  isResizing || (onStartResize(), (isResizing = !0)),
    o(contentTargetWidth + 2 * sidebarDefaultWidth) || o(1025)
      ...
      : t(
          1 * (contentTargetWidth + sidebarDefaultWidth),
          contentTargetWidth + 2 * sidebarDefaultWidth
        ) || t(769, 1024)
      ...
      ? ((deviceSize = "tablet"),
        ...
        leftSidebar &&
          ...
          !leftSidebar.collapsed &&
          rightSidebar.collapse(!0),
        leftSidebar && !fullyInitialized && leftSidebar.collapse(!0))
      : (n(2 * sidebarDefaultWidth) || n(480)) &&
        ((deviceSize = "phone"),
        document.body.classList.toggle("floating-sidebars", !0),
        document.body.classList.toggle("is-large-screen", !1),
        document.body.classList.toggle("is-small-screen", !1),
  ...
  checkStillResizingTimeout = setTimeout(function () {
    window.innerWidth == i &&
      ((checkStillResizingTimeout = void 0), (isResizing = !1), onEndResize());
  }, 200);
}
...
async function waitUntil(e, t = 100, o = 2e3) {
  return new Promise((n) => {
    ...
    i = setInterval(() => {
      e() && (clearInterval(i), clearTimeout(a), n());
    }, t);
  });
}
...
var slideToggle = (e, t = 500) =>
  ...
  slideToggleAll = (e, t = 500) =>
    "none" === window.getComputedStyle(e[0]).display
      ? slideDownAll(e, t)
      : slideUpAll(e, t);
function getURLExtention(e) {
  return e.split(".").pop().split("?")[0].split("#")[0].toLowerCase().trim();
}
let transferDocument = document.implementation.createHTMLDocument(),
  loading = !1;
async function loadDocument(e, t, o) {
  if (((e = decodeURI(e)), loading))
    return void console.log("Already loading document.");
  loading = !0;
  let n = new URL(e, absoluteBasePath);
  if (
    ((relativePathname = getVaultRelativePath(n.href)),
    console.log("Loading document: ", n.pathname),
    n.pathname == loadedURL?.pathname)
  )
    return (
      console.log("Document already loaded."),
      (loadedURL = n),
      setActiveDocument(loadedURL, !1, !1),
      await initializePage(),
      void (loading = !1)
    );
  loadedURL = n;
  let i,
    a = loadedURL.pathname;
  await showLoading(!0);
  try {
    i = await fetch(a);
  } catch (e) {
    return window.location.assign(a), void (loading = !1);
  }
  if (i.ok) {
    setActiveDocument(loadedURL, o, t);
    let n = getURLExtention(e);
    if (
      ("/" == n && (n = "html"),
      (documentType = "none"),
      (embedType = "none"),
      ...
      let t = transferDocument.querySelector(".document-container");
      documentContainer.innerHTML = t.innerHTML;
      let o = transferDocument.querySelector(".outline-tree");
      outlineTree && o && (outlineTree.innerHTML = o.innerHTML),
        (document.title = transferDocument.title),
        transferDocument.close();
    } else if (
      ((documentType = "embed"),
      (embedType = extentionToTag(n)),
      null != embedType)
    ) {
      ...
      downloadBlob(await i.blob(), e.split("/").pop());
    }
    await initializePage(),
      initializePageEvents(documentContainer),
      initializePageEvents(outlineTree);
  } else pageNotFound(viewContent);
  await showLoading(!1), (loading = !1);
}
function setActiveDocument(e, t, o, n = !0) {
  let i = getVaultRelativePath(e.href),
    a = decodeURI(i)
      .split("#")[0]
      .split("?")[0]
      .replace('"', '\\"')
      .replace("'", "\\'");
  ("/" != a && "" != a) || (a = "index.html");
  let l = document.querySelector(".file-tree .tree-item.mod-active"),
    s = document.querySelector(
      `.file-tree .tree-item:has(>.tree-link[href^="${a}"])`
    );
  if (
    (s &&
      !s.isEqualNode(l) &&
      (l?.classList.remove("mod-active"),
      s.classList.add("mod-active"),
      t && scrollIntoView(s, { block: "center", inline: "nearest" }, n)),
    "undefined" != typeof graphData && window.graphRenderer)
  ) {
    let e =
      graphData?.paths.findIndex(function (e) {
        return e.endsWith(a);
      }) ?? -1;
    e >= 0 && (window.graphRenderer.activeNode = e);
  }
  console.log("Active document: " + o),
    o &&
      "file:" != window.location.protocol &&
      (window.history.pushState({ path: i }, "", i),
      console.log("Pushed state: " + i));
}
function parseURLParams() {
  const e = loadedURL.searchParams.get("mark"),
    t = loadedURL.searchParams.get("query"),
    o = decodeURI(loadedURL.hash);
  if ((e && searchCurrentDocument(e), t && search(t), o)) {
    const e = document.getElementById(o.substring(1));
    e
      ? scrollIntoView(e, { behavior: "smooth", block: "start" })
      : console.log("Heading not found: " + o);
  }
}
async function showLoading(e) {
  if (
    ((documentContainer.style.transitionDuration = ""),
    loadingIcon.classList.toggle("show", e),
    documentContainer.classList.toggle("hide", e),
    e)
  ) {
    let e = getViewBounds();
    (loadingIcon.style.left = e.centerX - loadingIcon.offsetWidth / 2 + "px"),
      (loadingIcon.style.top = e.centerY - loadingIcon.offsetHeight / 2 + "px"),
...
function pageNotFound(e) {
  ...
  let t = getURLRootPath(absoluteBasePath + loadedURL.pathname);
  (relativeBasePath = t),
    (document.querySelector("base").href = t),
    (document.title = "Page Not Found");
}
function setupRootPath(e) {
  let t = e.getElementById("root-path");
  if (!t) return;
  let o = t.getAttribute("root-path"),
    n = document.createElement("base");
  (n.href = o),
    console.log("Setting root path: " + o),
    document.querySelector("base").replaceWith(n),
    document.querySelector("#root-path").setAttribute("root-path", o),
    (relativeBasePath = o),
    (absoluteBasePath = new URL(o, window.location.href).href);
}
function getURLPath(e = window.location.pathname) {
  return (
    null == absoluteBasePath && setupRootPath(document),
    e.replace(absoluteBasePath, "")
  );
}
...
function getVaultRelativePath(e) {
  return e.replace(absoluteBasePath, "");
}
...
async function collapseHeader(e, t, o = !0, n = !1) {
  ...
  let a = e.collapseHeight;
  function l(t = void 0) {
    if (null != t) e.markdownPreviewSizer.style.minHeight = t + "px";
    else {
      let t = Array.from(e.markdownPreviewSizer.children).reduce(
        (e, t) => e + t.offsetHeight,
        0
      );
      e.markdownPreviewSizer.style.minHeight = t + "px";
    }
  }
  if (
    ((i.style.height = a + "px"),
  ...
  let s = getComputedStyle(i).transitionDuration;
  s = s.endsWith("s")
    ? parseFloat(s)
    : s.endsWith("ms")
    ? parseFloat(s) / 1e3
    : 0;
  let r = Math.min((s * Math.sqrt(a)) / 16, 0.5);
  (i.style.transitionDuration = `${r}s`),
    (i.style.height = t ? "0px" : a + "px"),
    e.classList.toggle("is-animating", !0),
    e.classList.toggle("is-collapsed", t),
    e.markdownPreviewSizer.closest(".markdown-embed") && l(t ? 0 : void 0),
    setTimeout(function () {
      (i.style.transitionDuration = ""),
        t || (i.style.height = ""),
        e.classList.toggle("is-animating", !1),
...
function hideHeader(e) {
  ...
  if (e.classList.contains("is-hidden") || e.classList.contains("is-collapsed"))
    return;
...
function setupTrees(e) {
  e.querySelectorAll(".collapse-tree-button").forEach(function (e) {
    (e.treeRoot = e.closest(".tree-container")),
      (e.icon = e.firstChild),
      ...
      (e.collapse = function (t) {
        setTreeCollapsedAll(
          e.treeRoot.classList.contains("file-tree")
            ? fileTreeItems
            : outlineTreeItems,
          t
        ),
      ...
      (e.toggleState = function (t) {
        void 0 === t && (t = !e.collapsed), (e.collapsed = t), e.setIcon(t);
      }),
      e.addEventListener("click", function (t) {
        return t.preventDefault(), t.stopPropagation(), e.toggleCollapse(), !1;
      }),
      (e.treeRoot.classList.contains("file-tree")
        ? fileTreeItems
        : outlineTreeItems
        ...
        (e) =>
          !e.classList.contains("is-collapsed") &&
          e.classList.contains("mod-collapsible")
      ) && e.toggleState(!1);
  });
  let t = Array.from(
      e.querySelectorAll(
        ".tree-container.file-tree .tree-item:has(.collapse-icon) > .tree-link"
      )
    ),
    o = Array.from(
      e.querySelectorAll(
        ".tree-container.outline-tree .tree-item:has(.collapse-icon) > .tree-link .collapse-icon"
      )
    ),
    n = Array.from(t).concat(Array.from(o));
  for (let e of n) {
    let t = e?.closest(".tree-item");
    t &&
      e &&
      e?.addEventListener("click", function (e) {
        e.preventDefault(), e.stopPropagation(), toggleTreeCollapsed(t);
      });
  }
}
async function setTreeCollapsed(e, t, o = !0, n = !0) {
  if (
    (e.classList.contains("mod-collapsible") ||
      (e = e.closest(".mod-collapsible")),
    !e || !e.classList.contains("mod-collapsible"))
  )
    return;
  if (e.classList.contains("is-collapsed") == t) return;
  if (n) {
    let t = e.parentElement.closest(".mod-collapsible");
    t && (await setTreeCollapsed(t, !1, o, n));
  }
  let i = e.querySelector(".tree-item-children");
  if (t)
    e.classList.add("is-collapsed"),
      o ? slideUp(i, 100) : (i.style.display = "none");
  else {
    e.classList.remove("is-collapsed"),
      o ? slideDown(i, 100) : (i.style.display = "");
    let t = e.closest(".tree-container");
    if (t) {
      let e = t.querySelector(".collapse-tree-button");
      e && e.toggleState(!1);
    }
  }
}
async function setTreeCollapsedAll(e, t, o = !0) {
  ...
  e.forEach(async (e) => {
    ...
    t ? e.classList.add("is-collapsed") : e.classList.remove("is-collapsed"),
      n.push(o);
  }),
    t
      ? o
...
function toggleTreeCollapsed(e) {
  (e = e.closest(".tree-item")) &&
    setTreeCollapsed(e, !e.classList.contains("is-collapsed"));
}
function toggleTreeCollapsedAll(e) {
  e && setTreeCollapsedAll(e, !e[0].classList.contains("is-collapsed"));
}
function getFileTreeItemFromPath(e) {
  return document.querySelector(
    `.file-tree .tree-item:has(> .tree-link[href^="${e}"])`
  );
}
async function filterFileTree(e, t, o, n = !0) {
  n && (await setTreeCollapsedAll(fileTreeItems, !1, !1));
  let i = Array.from(
    document.querySelectorAll(".file-tree .tree-item:not(.filtered-out)")
  );
  for await (let l of i) l.classList.add("filtered-out");
  await removeTreeHintLabels();
  for (let s = 0; s < e.length; s++) {
    let r = e[s],
      c = t[s],
      d = getFileTreeItemFromPath(r);
    if (d) {
      d.classList.remove("filtered-out");
      let u = d.querySelector(".tree-link");
      u && (u.href = r + "?mark=" + o);
      let m = d.parentElement.closest(".tree-item");
      for (; m; )
        m.classList.remove("filtered-out"),
          (m = m.parentElement.closest(".tree-item"));
      if (c.length > 0) {
        let p = d
          .querySelector(".tree-link")
          .appendChild(document.createElement("div"));
        function a(e, t) {
          if ("" == (e = e.trim())) return;
          let o = document.createElement("a");
          o.classList.add("tree-hint-label"),
            o.classList.add("internal-link"),
            (o.textContent = e),
            (o.href = decodeURI(t).replaceAll(" ", "_")),
            p.appendChild(o);
        }
        p.classList.add("tree-hint-container");
        for (let h of c) a(h, r + "#" + h);
        setupLinks(p);
      }
    }
  }
}
async function clearFileTreeFilter(e = !0) {
  await removeTreeHintLabels();
  let t = document.querySelectorAll(".file-tree .filtered-out");
  for await (let e of t) e.classList.remove("filtered-out");
  let o = document.querySelectorAll(".file-tree .tree-link[href*='?mark=']");
  for await (let e of o) {
    let t = e.href.split("?")[0];
    (t = getVaultRelativePath(t)), (e.href = t);
  }
  e && (await setTreeCollapsedAll(fileTreeItems, !0, !1));
}
async function removeTreeHintLabels() {
  let e = document.querySelectorAll(".tree-hint-container");
  for await (let t of e) t.remove();
}
function sortFileTreeDocuments(e) {
  let t = Array.from(
    document.querySelectorAll(
      ".file-tree .tree-item.mod-tree-file:not(.filtered-out)"
    )
  );
  t.sort(e);
  for (let e = 1; e < t.length; e++) {
    let o = t[e],
      n = t[e - 1];
    o.parentElement == n.parentElement && n.after(o);
  }
  let o = Array.from(
    document.querySelectorAll(
      ".file-tree .tree-item.mod-tree-folder:not(.filtered-out)"
    )
  );
  o.sort(function (e, o) {
    let n = e.querySelector(".tree-item.mod-tree-file:not(.filtered-out)"),
      i = o.querySelector(".tree-item.mod-tree-file:not(.filtered-out)");
    return t.indexOf(n) - t.indexOf(i);
  });
  for (let e = 1; e < o.length; e++) {
    ...
    n || t.parentElement.prepend(t);
  }
}
function sortFileTree(e) {
  let t = Array.from(
    document.querySelectorAll(
      ".file-tree .tree-item.mod-tree-file:not(.filtered-out)"
    )
...
function sortFileTreeAlphabetically(e = !1) {
  sortFileTree(function (t, o) {
    const n = t.querySelector(".tree-item-title"),
      i = o.querySelector(".tree-item-title");
    if (!n || !i) return 0;
    const a = n.textContent.toLowerCase(),
      l = i.textContent.toLowerCase();
    return a.localeCompare(l, void 0, { numeric: !0 }) * (e ? -1 : 1);
  });
}
function setupLists(e) {
  let t = Array.from(e.querySelectorAll(".list-collapse-indicator"));
...
function setupCanvas(e) {
  ...
  setViewCenter(t.centerX, t.centerY),
    e.querySelectorAll(".canvas-node-container").forEach(function (e) {
      ...
      e.addEventListener("mouseenter", o), e.addEventListener("touchstart", o);
    }),
    ...
    canvasWrapper.addEventListener("mousedown", n),
    canvasWrapper.addEventListener("touchstart", n);
  let o = !1;
  function n(e) {
    let t = e.touches ?? [];
    if (
      ((o = !1),
      !(t.length > 1) && (1 == e.button || 0 == e.button || t.length > 0))
    ) {
      let n = getPointerPosition(e),
        i = !1,
        a = 0,
        l = t.length,
        s = function (t) {
          ...
          l != s.length && ((n = r), (l = s.length));
          let c = r.x - n.x,
            d = r.y - n.y;
          if ((1 == e.button || 1 == s.length) && focusedCanvasNode) {
            let e = Math.abs(c) > Math.abs(1.5 * d),
              t = Math.abs(d) > Math.abs(1.5 * c),
              n = focusedCanvasNode.querySelector(".markdown-preview-sizer");
            if (n) {
              let i = n.scrollHeight > n.parentElement.clientHeight + 1,
                a = n.scrollWidth > n.parentElement.clientWidth + 1;
              o = !(
                !((e && a) || (t && i)) ||
                !(window?.navigator?.platform?.startsWith("Win") ?? 1)
              );
            }
          }
          if (
        ...
        r = function (e) {
          document.body.removeEventListener("mousemove", s),
            ...
            document.body.removeEventListener("mouseenter", c),
            document.body.removeEventListener("touchmove", s),
            document.body.removeEventListener("touchend", r),
            document.body.removeEventListener("touchcancel", r),
            (o = !1);
        },
        c = function (e) {
          1 != e.buttons && 4 != e.buttons && r(e);
        };
      document.body.addEventListener("mousemove", s),
        document.body.addEventListener("mouseup", r),
        document.body.addEventListener("mouseenter", c),
        document.body.addEventListener("touchmove", s),
        document.body.addEventListener("touchend", r),
        document.body.addEventListener("touchcancel", r);
    }
  }
  let i = 0,
    a = 0;
  ...
  canvasWrapper.addEventListener("wheel", function (e) {
    ...
    if ((e.preventDefault(), e.stopPropagation(), r)) {
      ...
      s -= e.deltaY / 200;
      const o = 0.14 * l;
      (s = clamp(s, -o, o)), t && requestAnimationFrame(m);
    }
  });
  let c = 0,
    d = 0,
  ...
  function m(e) {
    ...
    let t = l;
    (l += s * (c / 1e3) * 30), (l = clamp(l, 0.1, 10));
    getViewBounds();
    scaleCanvasAroundPoint(l / t, i, a),
      (s *= 0.4),
      Math.abs(s) < 0.01 ? ((s = 0), (d = 0)) : requestAnimationFrame(m);
  }
...
function getNodesBounds() {
  ...
  canvasNodes.forEach(function (i) {
    ...
    a.x < e && (e = a.x),
      a.y < t && (t = a.y),
      a.x + a.width > o && (o = a.x + a.width),
      a.y + a.height > n && (n = a.y + a.height);
  });
  let i = o - e,
    a = n - t;
  return {
...
function scaleCanvasAroundPoint(e, t, o) {
  let n = getCanvasBounds(),
    ...
    l = t - (n.x + i * e),
    s = o - (n.y + a * e);
  return scaleCanvas(e), translateCanvas(l, s), { x: l, y: s };
}
...
function translateCanvas(e, t) {
  let o = canvas.style.translate,
    ...
    a = n.length > 1 ? parseFloat(o.split(" ")[1].trim()) : i;
  canvas.style.translate = `${i + e}px ${a + t}px`;
}
  ...
  e.querySelectorAll(".callout.is-collapsible .callout-title").forEach(
    function (e) {
      e.addEventListener("click", function () {
        ...
        t.classList.toggle("is-collapsed"),
          e.querySelector(".callout-fold").classList.toggle("is-collapsed"),
  ...
  e.querySelectorAll(".copy-code-button").forEach(function (t) {
    t.addEventListener("click", function () {
      ...
      navigator.clipboard.writeText(t),
        ...
        setTimeout(function () {
          e.querySelectorAll(".copy-code-button").forEach(function (e) {
            e.textContent = "Copy";
          });
        }, 2e3);
    });
...
function setupLinks(e) {
  e.querySelectorAll(
    ".internal-link, a.tag, .tree-link, .footnote-link"
  ).forEach(function (e) {
    e.addEventListener("click", function (t) {
      let o = e.getAttribute("href");
      if ((t.preventDefault(), t.stopPropagation(), !o))
        return void console.log("No target found for link");
      let n = relativePathname.split("#")[0].split("?")[0];
      (o.startsWith("#") || o.startsWith("?")) && (o = n + o),
        loadDocument(o, !0, !e.classList.contains("tree-link"));
    });
  });
}
function setupSidebars() {
  rightSidebar &&
      ...
      (e.collapsed = e.classList.contains("is-collapsed")),
        (e.collapse = function (t = !0) {
          if (
            ...
            !t && document.body.classList.contains("floating-sidebars"))
          ) {
            document.body.addEventListener("click", function t(o) {
              o.target.closest(".sidebar") ||
                (e.collapse(!0), document.body.removeEventListener("click", t));
            });
          }
          "phone" == deviceSize &&
            (t || e.otherSidebar.fullCollapse(!0, !0),
            t && e.gutter.otherGutter.collapse(!1, !0)),
            "tablet" == deviceSize && (t || e.otherSidebar.collapse(!0)),
        ...
        (e.fullCollapse = function (e = !0, t = !1) {
          this.collapse(e), this.gutter.collapse(!0, t), (this.collapsed = e);
        }),
        (e.toggleCollapse = function () {
    ...
    sidebarGutters.forEach(function (e) {
      (e.collapsed = e.classList.contains("is-collapsed")),
        (e.collapse = function (e, t = !1) {
          t && (this.classList.toggle("is-collapsed", e), (this.collapsed = e));
        }),
        (e.toggleCollapse = function () {
          this.collapse(!this.collapsed);
        });
    ...
    isMobile() || setupSidebarResize());
}
function setupSidebarResize() {
  let e = document.querySelector(".sidebar-left .sidebar-handle"),
    t = document.querySelector(".sidebar-right .sidebar-handle");
  if (!e || !t) return;
  let o = null,
    n = 15 * parseFloat(getComputedStyle(e.parentElement).fontSize),
    i = n / 4,
    a = localStorage.getItem("sidebar-right-width"),
    l = localStorage.getItem("sidebar-left-width");
  function s(e) {
    if (!o) return;
    var t = o.classList.contains("sidebar-left")
        ? e.clientX
        : window.innerWidth - e.clientX,
      a = `min(max(${t}px, 15em), 40vw)`;
    t < i
      ? (o.collapse(!0), o.style.removeProperty("transition-duration"))
      : (o.collapse(!1),
        o.style.setProperty("--sidebar-width", a),
        t > n && (o.style.transitionDuration = "0s"));
  }
  function r(e) {
    (o = e.target.closest(".sidebar")),
      o.classList.add("is-resizing"),
      document.addEventListener("pointermove", s),
      document.addEventListener("pointerup", function () {
        document.removeEventListener("pointermove", s);
        var e = getComputedStyle(o).getPropertyValue("--sidebar-width");
        let t = o.classList.contains("sidebar-left");
        localStorage.setItem(
          t ? "sidebar-left-width" : "sidebar-right-width",
          e
        ),
  ...
  function c(e) {
    ...
    if (t) {
      t.style.removeProperty("transition-duration"),
        t.style.removeProperty("--sidebar-width");
      let e = t.classList.contains("sidebar-left");
      localStorage.removeItem(e ? "sidebar-left-width" : "sidebar-right-width");
    }
  }
  a &&
    document
      .querySelector(".sidebar-right")
      .style.setProperty("--sidebar-width", a),
    l &&
      document
        .querySelector(".sidebar-left")
        .style.setProperty("--sidebar-width", l),
...
function setupThemeToggle() {
  function e(e, t = !1) {
    if (((themeToggle.checked = e), t)) {
      var o = document.body.style.transition;
      document.body.style.transition = "none";
    }
    !themeToggle.classList.contains("is-checked") && e
      ? themeToggle.classList.add("is-checked")
      : themeToggle.classList.contains("is-checked") &&
        !e &&
  ...
  themeToggle &&
    (null != localStorage.getItem("theme") &&
      e("light" == localStorage.getItem("theme")),
    document.body.classList.contains("theme-light") ? e(!0) : e(!1),
    document
      .querySelector(".theme-toggle-input")
      ?.addEventListener("change", (t) => {
        let o = !("light" == localStorage.getItem("theme"));
        console.log("Theme toggle changed to: " + o), e(o);
      }));
}
let index,
  searchResults,
  flashElement = null,
  flashAnimation = null;
function scrollIntoView(e, t, o = !0) {
  setTreeCollapsed(e, !1, o);
  flashElement && (flashElement.remove(), flashAnimation.cancel()),
    (flashElement = document.createElement("div")),
    flashElement.classList.add("scroll-highlight"),
    e.appendChild(flashElement),
    t
      ? flashElement.scrollIntoView({ behavior: o ? "smooth" : "auto", ...t })
      : flashElement.scrollIntoView({ behavior: o ? "smooth" : "auto" });
  var n = e.style.position;
  (e.style.position = "relative"),
    (flashAnimation = flashElement.animate(
      [
        { opacity: 0 },
        { opacity: 0.8 },
        { opacity: 0.8 },
      ...
      { duration: 1500, iterations: 1, delay: 300 }
    )),
    (flashAnimation.onfinish = function () {
      flashElement.remove(), (e.style.position = n);
    });
}
function setupScroll(e) {
  ...
  t.forEach(async function (e) {
    ...
    e.updateVisibleWindowMarkdown = function (o = !0, i = !0) {
      ...
      n = Math.min(0.1 * a.height, 150);
      let l = a.top - n,
        s = a.bottom + n;
      async function r(e) {
        let t = e?.getBoundingClientRect();
        if (!t) return;
        let n = (t.top < l && t.bottom < l) || (t.top > s && t.bottom > s);
...
async function setupSearch() {
  ...
  if (
    ...
    return;
  const e = await fetch("lib/search-index.json"),
    t = await e.text();
  index = MiniSearch.loadJSON(t, {
    fields: ["title", "path", "tags", "headers"],
  });
  document
    .querySelector(".search-input-clear-button")
    .addEventListener("click", (e) => {
      search("");
    }),
    searchInput.addEventListener("input", (e) => {
      const t = e.target.value ?? "";
      startsWithAny(t, ["#", "tag:", "title:", "name:", "header:", "H:"])
...
async function search(e) {
  searchInput.value = e;
  let t = ["title", "content", "tags", "headers", "path"];
  if (
    (e.startsWith("#") && (t = ["tags", "headers"]),
    e.startsWith("tag:") &&
      ((e = e.substring(e.indexOf(":") + 1)), (t = ["tags"])),
    startsWithAny(e, ["title:", "name:"]) &&
      ((e = e.substring(e.indexOf(":") + 1)), (t = ["title"])),
    startsWithAny(e, ["header:", "H:"]) &&
      ((e = e.substring(e.indexOf(":") + 1)), (t = ["headers"])),
    startsWithAny(e, ["path:"]) &&
      ((e = e.substring(e.indexOf(":") + 1)), (t = ["path"])),
    e.length >= 1)
  ) {
    const o = index.search(e, {
      prefix: !0,
      fuzzy: 0.3,
      boost: { title: 4, headers: 3, tags: 2, path: 1 },
      fields: t,
    });
    let n = [],
      i = [];
    for (let t of o) {
      if (
        ((t.score < 0.33 * o[0].score || n.length > 12) && n.length > 3) ||
        t.score < 0.1 * o[0].score
      )
        break;
      n.push(t.path);
      let a = [],
        l = !1;
      for (match in t.match) {
        if (t.match[match].includes("headers"))
          for (let o of t.headers)
            if (
              o.toLowerCase().includes(match.toLowerCase()) &&
              (a.push(o), e.toLowerCase() != match.toLowerCase())
            ) {
              l = !0;
              break;
            }
    ...
    if (document.querySelector(".file-tree"))
      filterFileTree(n, i, e).then(() =>
        sortFileTreeDocuments((e, t) => {
          if (!e || !t) return 0;
          let o = getVaultRelativePath(e.firstChild.href),
            i = getVaultRelativePath(t.firstChild.href);
          return (
            n.findIndex((e) => o.startsWith(e)) -
            n.findIndex((e) => i.startsWith(e))
          );
        })
      );
    else {
      const t = document.createElement("div");
      o.slice(0, 10).forEach((o) => {
        const n = document.createElement("div");
        n.classList.add("search-result");
        const i = document.createElement("a");
        i.classList.add("tree-link");
        const a = o.path + "?mark=" + encodeURIComponent(e);
        i.setAttribute("href", a),
          i.appendChild(document.createTextNode(o.title)),
          n.appendChild(i),
          t.append(n);
      }),
        searchResults.replaceChildren(t),
...
function startsWithAny(e, t) {
  for (let o = 0; o < t.length; o++) if (e.startsWith(t[o])) return !0;
  return !1;
}
async function searchCurrentDocument(e) {
  clearCurrentDocumentSearch();
  getTextNodes(
    document.querySelector(".markdown-preview-sizer") ?? documentContainer
  ).forEach(async (t) => {
    const o = t.nodeValue,
      n = o.replace(new RegExp(e, "gi"), (e) => `<mark>${e}</mark>`);
    if (n !== o) {
      const e = document.createElement("div");
      e.innerHTML = n;
      Array.from(e.childNodes).forEach((e) => {
        e.nodeType != Node.TEXT_NODE && e.setAttribute("class", "search-mark"),
          t.parentNode.insertBefore(e, t);
      }),
        t.parentNode.removeChild(t);
    }
  });
  let t = document.querySelector(".search-mark");
  setTimeout(() => {
    t && scrollIntoView(t, { behavior: "smooth", block: "start" });
  }, 500);
}
...
function getTextNodes(e) {
  const t = [],
    o = document.createTreeWalker(e, NodeFilter.SHOW_TEXT, null, !1);
  let n;
  for (; (n = o.nextNode()); ) t.push(n);
  return t;
}
